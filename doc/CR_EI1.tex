...
\vspace{5mm}
\noindent
\textbf{Ensembles et Indices :}
\begin{itemize}
    \item $I = $ L'ensemble des lignes (contraintes).
    \item $J = $ L'ensemble des colonnes (variables).
\end{itemize}

\noindent
\textbf{Paramètres (Données) :}
\begin{itemize}
    \item $c_j$ : Le coût de la colonne $j \in J$.
    \item $A = (a_{ij})$ : Une matrice binaire où $a_{ij} = 1$ si la ligne $i$ est couverte par la colonne $j$, et 0 sinon.
\end{itemize}

\noindent
\textbf{Variables de Décision :}
\begin{itemize}
    \item $x_j$ : Une variable binaire qui vaut 1 si la colonne $j$ est sélectionnée dans la solution, et 0 sinon.
\end{itemize}
...
\noindent
Notre modélisation JuMP traduit directement la formulation mathématique. La fonction \texttt{setSPP} prend la matrice des coûts \texttt{C} et la matrice des contraintes \texttt{A} en entrée et crée un modèle JuMP.

\vspace{3mm}
\noindent
\textbf{Explication des lignes clés de la fonction :}
\begin{itemize}
    \item \texttt{@variable(spp, x[1:n], Bin)} : Déclare les $n$ variables de décision $x_j$ et indique qu'elles sont binaires ($x_j \in \{0, 1\}$).
    \item \texttt{@objective(spp, Max, dot(C, x))} : Définit la fonction objectif. On cherche à \texttt{Max}imiser le produit scalaire entre les coûts \texttt{C} et les variables \texttt{x} (ce qui équivaut à $\max \sum c_j x_j$).
    \item \texttt{@constraint(spp , ...)} : Crée une contrainte pour chaque ligne $i$ (de 1 à $m$), garantissant que la somme des $a_{ij} x_j$ sur cette ligne ne dépasse pas 1 ($\sum a_{ij} x_j \le 1, \forall i$).
\end{itemize}
...
Sur le plan des temps de résolution, le rapport est en faveur de notre heuristiques. Pour l'instance \texttt{pb\_1000rnd0300.dat}, notre heuristique complète ($t_{\text{glouton}} + t_{\text{local}}$) prend 0.142 secondes. Le solveur lui, prendrait plusieurs minutes. Le rapport est donc très largement en faveur de l'heuristique.
...
Oui, le recours aux métaheuristiques paraît prometteur. En partant déjà d'une solution gloutonne qui est très rapide à obtenir, on devrait pouvoir améliorer nos résultats grâce à des algorithmes comme GRASP et ses amélioration.
